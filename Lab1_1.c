...
//pthread_cond_t cond_running[4];  //本方向是否有车正在道路上行驶
...
void* CarThreadFun(void* info)
{
    ...
    //在旧版本中使用flag[dir]可能导致死锁，因此曾尝试使用条件变量，但flag的方式通过了测试，该方式未果
	// pthread_cond_wait(&cond_running[dir],&mutex_dir[dir]);  //若本方向上有车正在通行，则等待
	// pthread_cond_wait(&cond_running[(dir+1)%4],&mutex_dir[dir]);  //若右侧有车正在通行，则等待
	// pthread_cond_wait(&cond_running[(dir+3)%4],&mutex_dir[dir]);  //若左侧有车正在通行，也需要等待（虽然右侧车优先级高于左侧车，但是不应当抢占）
    ...
    //发出信号令左侧的车辆通行 
	//此处如果增加pthread_mutex_lock这段可能导致死锁
